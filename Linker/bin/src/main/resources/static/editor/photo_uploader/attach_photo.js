    ...
    function html5Upload() {	
    	...
    	var writer = url.searchParams.get("writer");
    	
    	sUploadURL= '/file/upload'; 	//upload URL
    	...
    	for(var j=0, k=0; j < nImageInfoCnt; j++) {
    		...
    		try{
	    		if(!!tempFile){
		    		form.append('tempFile', tempFile);
		    		form.append('writer', writer);
	    			//Ajax통신하는 부분. 파일과 업로더할 url을 전달한다.
	    			callAjaxForHTML5(form,sUploadURL);
	    			k += 1;
	    		}
	    	}catch(e){}
    		tempFile = null;
    ...
    function callAjaxForHTML5 (form, sUploadURL){
			...
			if(xhr.readyState === 4){
				if(xhr.status === 200){
					...
					if(sResString.indexOf("NOTALLOW_") > -1){
						...
						var json = JSON.parse(sResString);
						makeArrayFromString(json);
					}
				} else {
					alert("요청 실패");
				}
		...
		oAjax.header("Content-Type", false);
    ...
    function makeArrayFromString(sResString){
    	...
    	aResult[aResultleng] = sResString;
